...
const registerUser = asyncHandler(async (req, res) => {
    const { firstname, lastname, email, password, phone, country, specialization, isAdmin } = req.body
    // 
    ...
    const Tempuser = { email, firstname, lastname, password: hashedpassword, phone, country, isAdmin }
    const user = await User.create(Tempuser)
    // 
    const token = jwt.sign({ userId: user._id, isAdmin: user.isAdmin, firstname: user.firstname, lastname: user.lastname }, process.env.JWT_CODE, { expiresIn: '30d' })
