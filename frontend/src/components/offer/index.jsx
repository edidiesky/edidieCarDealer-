...
export default function OfferIndex() {
  return (
    <div className="w-100 flex column gap-2">
      <Banner title={"Latest Offers"} />
      <div className="py-6 w-90 auto flex column gap-4">
        {offerData.map((x, index) => {
          return (
            <div
              className="w-100 grid grid-2 item-start gap-2 py-2 justify-space"
              key={index}
            >
              <div className="w-100 flex column gap-2">
                <div className="w-100 flex gap-2 column">
                  ...
                  <h2 className="fs-30 text-extra-bold text-dark">
                    {x.subInfo}
                  </h2>
                  <div className="w-100">
                    <button
                      className="fs-12 text-bold text-dark"
                      style={{ padding: "1.4rem", background: "#E8EFF9" }}
                    >
                      {x.discount}
                    </button>
                  </div>
